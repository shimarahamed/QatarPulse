...
function FavoritesList() {
    ...
    const [favoriteBusinesses, setFavoriteBusinesses] = useState<WithId<Business>[]>([]);
    ...
    const { data: favoriteList, isLoading: isLoadingList } = useDoc(favoritesListRef);

    useEffect(() => {
        if (favoriteList?.businessIds?.length > 0) {
            const fetchBusinesses = async () => {
                const businessesRef = collection(firestore, "businesses");
                const q = query(businessesRef, where('__name__', 'in', favoriteList.businessIds));
                const querySnapshot = await getDocs(q);
                const businesses = querySnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as WithId<Business>));
        ...
        return (
             <Card className="border-dashed">
                <CardContent className="p-10 text-center">
                    ...
                    <h3 className="mt-4 text-lg font-medium text-muted-foreground">Sign In Required</h3>
